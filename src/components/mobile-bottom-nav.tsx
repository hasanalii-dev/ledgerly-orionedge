...
export function MobileBottomNav() {
  ...
  return (
    <div className={cn(
      ...
      {/* Plain Dark Black Glass Dock Container with Backdrop Blur */}
      <div className="flex items-center justify-between px-3 py-2 rounded-[2.5rem] bg-black/85 backdrop-blur-2xl backdrop-saturate-[180%] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.9)] relative overflow-hidden">
        {links.map((link) => {
            ...
            >
              {isActive && (
                <>
                  ...
                  <div className="absolute inset-x-1 inset-y-1 rounded-2xl bg-[#3DDC97]/15 border border-[#3DDC97]/40 shadow-[0_0_15px_rgba(61,220,151,0.25)] backdrop-blur-md" />
                  {/* Active Bottom Glow Dot */}
                  <div className="absolute bottom-1 w-1.5 h-1.5 rounded-full bg-[#3DDC97] shadow-[0_0_8px_#3DDC97]" />
                </>
              )}
              <Icon className={cn(
                "w-5 h-5 relative z-10 transition-all duration-300",
                isActive ? "scale-110 text-[#3DDC97] drop-shadow-[0_0_8px_rgba(61,220,151,0.7)]" : "group-hover:scale-105"
              )} />
            </Link>
          );
        })}

...
function DashboardPage() {
  ...
  const { data: urgentRemindersCount = 0 } = useQuery({
    ...
    queryFn: async () => {
      const [{ data: invoices }, { data: loans }, { data: states }] = await Promise.all([
        supabase.from("invoices").select("id, due_date, status").eq("planner_id", plannerId),
        supabase.from("loans" as any).select("id, due_date, remaining_amount").eq("planner_id", plannerId),
        supabase.from("activity_events").select("*").eq("planner_id", plannerId).eq("kind", "notification_state"),
      ]);
      ...
      (states ?? []).forEach((st: any) => {
        ...
        if (meta.status === "dismissed") {
          hiddenSet.add(st.ref_id);
        } else if (meta.status === "snoozed" && meta.snooze_until && meta.snooze_until > todayStr) {
          hiddenSet.add(st.ref_id);
        }
      });
      ...
      (invoices ?? []).forEach((inv: any) => {
        if (!hiddenSet.has(`inv_${inv.id}`) && inv.status !== "paid" && inv.due_date) {
      ...
      (loans ?? []).forEach((l: any) => {
        if (!hiddenSet.has(`loan_${l.id}`) && l.due_date && Number(l.remaining_amount || 0) > 0) {
            ...
            >
              <Link to="/app/p/$plannerId/notifications" params={{ plannerId }}>
                ...
                {totalNotificationBadgeCount > 0 && (
                  <span className="absolute -top-1 -right-1 h-4 w-4 bg-[#FF3366] text-white text-[9px] font-bold rounded-full border border-white/20 flex items-center justify-center shadow-md">

  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard label="Total Net" value={formatMoney(totalNet)} accent />
        <StatCard label="Best Month" value={formatMoney(Math.max(0, ...data.map((d) => d.net)))} />
...
function StatCard({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-2xl border border-hairline bg-card p-5">
      ...
      <div className={`mt-2 font-display text-3xl ${accent ? "text-primary" : ""}`}>{value}</div>
    </div>
  );
}
